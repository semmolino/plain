...
export function ClarificationThread({ a }: { a: Absence }) {
  ...
  if (log.length === 0 && a.DECISION_NOTE) {
    return <div style={{ fontSize: 12, color: 'var(--warning)', marginTop: 4 }}>Rückfrage: {a.DECISION_NOTE}</div>
  }
  ...
  return (
    <div>
      ...
      {bal && !!bal.atRisk && bal.atRisk > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--warning)', background: 'var(--warning-bg)', border: '1px solid var(--warning-bg)', borderRadius: 6, padding: '6px 10px', marginBottom: 12 }}>
      ...
      {showForm && (
        <div style={{ border: '1px solid var(--border)', borderRadius: 6, padding: 12, marginBottom: 12 }}>
          ...
          {msg && <div style={{ fontSize: 12, color: 'var(--danger-strong)', marginBottom: 8 }}>{msg}</div>}
