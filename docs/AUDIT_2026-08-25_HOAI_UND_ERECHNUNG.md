...
- **S1 — `services_bt_mapping.js` ist toter Code.** ✅ **BEHOBEN 17.09.2026**, aber anders als der Befund vorschlug. `loadBtMapping` hat im gesamten Repository **keinen Aufrufer**; die Mappingdatei liegt vor. Falls reaktiviert, sind die bekannten Schwächen real: `normalizeBt` (`:12-15`) matcht das BT-Muster an beliebiger Stelle, sodass `XBT-1` zu `BT-1` wird; die verbreiteten Schreibweisen mit Leerzeichen, Unterstrich oder Halbgeviertstrich (Excel-Autokorrektur) liefern `null`; `:62` verwirft solche Zeilen ohne Zähler oder Log. Fehlende Zeilen sind ununterscheidbar von nie existierenden. Empfehlung: verankertes Muster plus Rückgabe der verworfenen Zeilennummern. `_cache` (`:10`) wird nie invalidiert.

  **Der Befund nennt den Lader als Problem; das eigentliche Problem war die Tabelle.** Ein gehärteter Parser hätte die Excel-Zeilen zuverlässiger gelesen und damit genau nichts gewonnen — die Datei wurde ja von niemandem gebraucht. Ihr Zweck war Dokumentation, und als Dokumentation war sie unbeweisbar: sie konnte behaupten, BT-11 komme aus `PROJECT.ABBR`, ohne zu wissen, ob das Feld je im XML landet. Genau das war zu diesem Zeitpunkt der Fall (BT-11 wurde geladen und nie ausgegeben), und weder Tabelle noch Test haben es bemerkt. Dazu: im `git diff` nur „binary files differ", also im Review unlesbar — und bereits inhaltlich falsch (BT-13 und BT-14 zeigten beide auf `CONTRACT.ABBR`; im Code kommt BT-13 aus `BUYER_ORDER_REFERENCE`, BT-14 existiert nicht).

  **Umgesetzt:** `backend/config/Mapping BT.xlsx` und `services_bt_mapping.js` sind entfernt. An ihre Stelle tritt `backend/einvoice/` mit `btRegistry.js` als einziger Quelle der Wahrheit, `codelists.js` für die Wertevorräte der Norm (UNTDID 1001/5305/4461/4451, UN/ECE Rec. 20) und `profiles.js` für die Profil-Kennungen, die vorher auf beide Builder verteilt lagen.

  **Der Unterschied ist nicht das Dateiformat, sondern die Beweisbarkeit.** `registryCheck.js` erzeugt aus zwei Musterbelegen (Regelsteuersatz und §13b Reverse Charge) echtes CII- und UBL-XML und hält jede Registry-Zeile dagegen — **in beide Richtungen**: eine Zeile, deren Pfad im Dokument fehlt, ist ein Fehler; ein Element im Dokument ohne Zeile ebenso. Die zweite Richtung ist die, die keine gepflegte Tabelle je hat. Dazu geprüft: jede BT-Nummer im Quelltext der E-Rechnungs-Dateien und jedes `btField` einer Validator-Regel muss in der Registry existieren. Läuft als `tests/einvoice_mapping.test.js` bei jedem Push und als `npm run einvoice:check`.

  **Gegenprobe gemacht:** ein umbenanntes CII-Element (`ram:ID` → `ram:IDX` unter `SpecifiedProcuringProject`) meldet beide Richtungen — die Zeile ohne Vorkommen und das Element ohne Zeile. Eine erfundene BT-Nummer im Validator (`BT-999`) meldet zwei weitere.

  Die Tabelle für Menschen bleibt: `docs/EINVOICE_BT_MAPPING.md` wird mit `npm run einvoice:gen` aus der Registry erzeugt, samt Abdeckungszählung (82 Felder ausgegeben, 3 geladen aber ungenutzt, 61 bewusst nicht unterstützt) und den Codelisten. Dieselbe Bauart wie bei `license:gen` — die erzeugte Datei gehört mit ins Commit, sonst ist sie im Review nicht zu sehen, und das war der halbe Grund, die Excel loszuwerden.
...
- **S3 — Geladene, nie geschriebene Felder.** 🔶 **SICHTBAR GEMACHT 17.09.2026, fachlich offen.** `seller.creditorId` (BT-90), `seller.postOfficeBox`, `buyer.debitorNumber` (BT-46) werden aus der DB geholt und von keinem Builder ausgegeben. BT-46 wäre der naheliegende Kandidat, um N9 zu entschärfen.

  **Behoben ist bisher nur die Unsichtbarkeit.** Alle drei stehen jetzt in der BT-Registry mit Status `loaded-unused` und Begründung (BT-90, BT-36 für das Postfach, BT-46) und erscheinen in `docs/EINVOICE_BT_MAPPING.md` unter „geladen, aber von keinem Builder ausgegeben". Der Drift-Check verlangt für jeden solchen Eintrag eine Begründung; der Test zählt sie mit. Vorher waren sie nur zu finden, indem man `data.js` gegen beide Builder las.

  **Ob sie ausgegeben werden sollen, ist keine Aufräumfrage.** BT-90 (Gläubiger-ID) ergibt erst zusammen mit einer Lastschrift-Zahlungsart Sinn — die Zahlungsart ist derzeit fest 58. BT-46 als Entschärfung von N9 wäre eine inhaltliche Aussage über die Identität des Käufers und gehört entschieden, nicht nebenbei ergänzt.
