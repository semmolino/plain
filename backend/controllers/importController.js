...
// Strukturvorlage, bereits mit den eigenen Projekten und HOAI-Phasen gefuellt.
async function getStructurePrefill(req, res, supabase) {
  try {
    const { buffer, filename } = await svc.buildStructurePrefill({ supabase, tenantId: req.tenantId });
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);
    res.send(buffer);
  } catch (e) { fail(res, e); }
...
module.exports = { getDomains, getTemplate, getStructurePrefill, postPreview, postCommit, postErrorReport, getBatches, postRollback };
