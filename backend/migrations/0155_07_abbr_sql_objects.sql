...
DROP VIEW IF EXISTS "REPORTING"."VW_REPORT_PROJECT_DETAIL_STRUCTURE";
...
DROP FUNCTION IF EXISTS "public"."fn_project_report_structure"(p_tenant_id bigint, p_project_id bigint, p_as_of timestamp with time zone, p_date_from date, p_date_to date);
...
DROP FUNCTION IF EXISTS "public"."fn_project_list_report"(p_tenant_id bigint, p_as_of timestamp with time zone, p_date_from date, p_date_to date);
...
AS $function$
  select
    coalesce(ps."ABBR", 'Kein Status') as "STATUS_NAME",
...
CREATE FUNCTION public.fn_project_list_report(p_tenant_id bigint, p_as_of timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date)
 RETURNS TABLE("PROJECT_ID" bigint, "ABBR" text, "NAME_LONG" text, "PROJECT_STATUS_ID" bigint, "PROJECT_STATUS_NAME_SHORT" text, "PROJECT_TYPE_ID" bigint, "PROJECT_TYPE_NAME_SHORT" text, "PROJECT_MANAGER_ID" bigint, "PROJECT_MANAGER_DISPLAY" text, "ADDRESS_ID" bigint, "ADDRESS_NAME" text, "COMPANY_ID" bigint, "COMPANY_NAME" text, "DEPARTMENT_ID" bigint, "DEPARTMENT_NAME" text, "BUDGET_TOTAL_NET" numeric, "LEISTUNGSSTAND_PERCENT" numeric, "LEISTUNGSSTAND_VALUE" numeric, "HOURS_TOTAL" numeric, "COST_TOTAL" numeric, "COST_RATIO" numeric, "REMAINING_BUDGET_NET" numeric, "BILLED_NET_TOTAL" numeric, "OPEN_NET_TOTAL" numeric, "PAYED_NET_TOTAL" numeric, "SALES_TOTAL" numeric, "QTY_EXT_TOTAL" numeric)
 LANGUAGE sql
 STABLE
AS $function$
  WITH
...
CREATE FUNCTION public.fn_project_report_header(p_tenant_id bigint, p_project_id bigint, p_as_of timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date)
 RETURNS TABLE("TENANT_ID" bigint, "PROJECT_ID" bigint, "ABBR" text, "NAME_LONG" text, "PROJECT_STATUS_NAME_SHORT" text, "PROJECT_MANAGER_DISPLAY" text, "COMPANY_NAME" text, "BUDGET_TOTAL_NET" numeric, "LEISTUNGSSTAND_PERCENT" numeric, "LEISTUNGSSTAND_VALUE" numeric, "HOURS_TOTAL" numeric, "COST_TOTAL" numeric, "EARNED_VALUE_NET" numeric, "COST_RATIO" numeric, "REMAINING_BUDGET_NET" numeric, "ADVANCE_INVOICE_NET_TOTAL" numeric, "INVOICE_NET_TOTAL" numeric, "BILLED_NET_TOTAL" numeric, "OPEN_NET_TOTAL" numeric, "PAYED_NET_TOTAL" numeric, "SALES_TOTAL" numeric, "QTY_EXT_TOTAL" numeric)
 LANGUAGE sql
 STABLE
AS $function$
  WITH
...
CREATE FUNCTION public.fn_project_report_structure(p_tenant_id bigint, p_project_id bigint, p_as_of timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date)
 RETURNS TABLE("TENANT_ID" bigint, "PROJECT_ID" bigint, "STRUCTURE_ID" bigint, "PARENT_STRUCTURE_ID" bigint, "ABBR" text, "NAME_LONG" text, "IS_LEAF" boolean, "HOURS_TOTAL" numeric, "COST_TOTAL" numeric, "EARNED_VALUE_NET" numeric, "HONORAR_NET" numeric, "REST_HONORAR" numeric, "LEISTUNGSSTAND_PERCENT" numeric, "KOSTENQUOTE" numeric)
 LANGUAGE sql
 STABLE
AS $function$
  WITH
...
CREATE VIEW "REPORTING"."VW_REPORT_PROJECT_DETAIL_STRUCTURE" AS
 SELECT ps."TENANT_ID",
    ps."PROJECT_ID",
    ps."ID" AS "STRUCTURE_ID",
    ...
    COALESCE(sum(t."COST_TOTAL"), 0::numeric) AS "COST_TOTAL",
    COALESCE(ppl."REVENUE_COMPLETION", 0::numeric) + COALESCE(ppl."EXTRAS_COMPLETION", 0::numeric) AS "EARNED_VALUE_NET"
   FROM "PROJECT_STRUCTURE" ps
     LEFT JOIN "BOOKING" t ON t."TENANT_ID" = ps."TENANT_ID" AND t."STRUCTURE_ID" = ps."ID"
     LEFT JOIN "REPORTING"."VW_PROJECT_PROGRESS_LATEST" ppl ON ppl."TENANT_ID" = ps."TENANT_ID" AND ppl."STRUCTURE_ID" = ps."ID"
  GROUP BY ps."TENANT_ID", ps."PROJECT_ID", ps."ID", ps."FATHER_ID", ps."ABBR", ps."NAME_LONG", ppl."REVENUE_COMPLETION", ppl."EXTRAS_COMPLETION";
 ...
 SELECT ps."TENANT_ID",
    ...
    COALESCE(ps."REVENUE", 0::numeric) + COALESCE(ps."EXTRAS", 0::numeric) AS "HONORAR_NET",
        ...
        CASE
            ...
            ELSE COALESCE(sum(t."COST_TOTAL"), 0::numeric) /
            CASE
                WHEN ps."BILLING_TYPE_ID" = 2 THEN COALESCE(ps."REVENUE", 0::numeric)
                ELSE COALESCE(ps."REVENUE_COMPLETION", 0::numeric) + COALESCE(ps."EXTRAS_COMPLETION", 0::numeric)
            END
   ...
   FROM "PROJECT_STRUCTURE" ps
     LEFT JOIN "BOOKING" t ON t."TENANT_ID" = ps."TENANT_ID" AND t."STRUCTURE_ID" = ps."ID"
  GROUP BY ps."TENANT_ID", ps."PROJECT_ID", ps."ID", ps."FATHER_ID", ps."ABBR", ps."NAME_LONG", ps."BILLING_TYPE_ID", ps."REVENUE", ps."EXTRAS", ps."REVENUE_COMPLETION", ps."EXTRAS_COMPLETION", ps."REVENUE_COMPLETION_PERCENT";
