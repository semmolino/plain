-- Migration 0030: Fix report correctness
--
-- Problems fixed:
--   1. BT2 (hourly) elements show 0 Leistungsstand in "Aktuell" mode
--      → VW_PROJECT_PROGRESS_AGG and VW_REPORT_PROJECT_DETAIL_STRUCTURE now use
--        PROJECT_STRUCTURE.REVENUE as earned value for BT2 leaves.
--
--   2. Date-filtered RPCs (Stichtag / Zeitraum):
--      a. HONORAR shows current budget even before project existed
--         → Budget now comes from last non-null PROJECT_PROGRESS.REVENUE ≤ cutoff
--           (falls back to PROJECT_STRUCTURE if no snapshot exists yet).
--      b. Leistungsstand 0 for BT2 in date-filtered mode
--         → BT2 completion = TEC.SP_TOT filtered to the period.
--      c. Completion NULL because a payment/invoice PP row was the latest row
--         → prog CTE now uses LATERAL to find last NON-NULL REVENUE_COMPLETION
--           independently of other columns (carries forward past NULL rows).
--      d. Abgerechnet (Netto) ignores date filter
--         → Now sums PARTIAL_PAYMENT + INVOICE rows filtered by document date.
--      e. Bezahlt ignores date filter
--         → Now sums PAYMENT rows filtered by PAYMENT_DATE.
--
...
-- ── 1. VW_PROJECT_PROGRESS_AGG — fix BT2 completion ─────────────────────────
-- For BT2 leaf nodes earned = full budget (all booked TEC is earned work).
-- For BT1 leaf nodes earned = REVENUE_COMPLETION / EXTRAS_COMPLETION as before.
...
SELECT
  ...
  CASE
    ...
    ELSE 100.0
       * ( SUM(CASE WHEN ps."BILLING_TYPE_ID" = 2 THEN COALESCE(ps."REVENUE", 0)
                    ELSE COALESCE(ps."REVENUE_COMPLETION", 0) END)
         + SUM(CASE WHEN ps."BILLING_TYPE_ID" = 2 THEN 0
                    ELSE COALESCE(ps."EXTRAS_COMPLETION",  0) END) )
       / ( SUM(COALESCE(ps."REVENUE", 0)) + SUM(COALESCE(ps."EXTRAS", 0)) )
  END                              AS "LEISTUNGSSTAND_PERCENT",

  SUM(CASE WHEN ps."BILLING_TYPE_ID" = 2 THEN COALESCE(ps."REVENUE", 0)
           ELSE COALESCE(ps."REVENUE_COMPLETION", 0) END) AS "REVENUE_COMPLETION_VALUE",

  SUM(CASE WHEN ps."BILLING_TYPE_ID" = 2 THEN 0
           ELSE COALESCE(ps."EXTRAS_COMPLETION",  0) END) AS "EXTRAS_COMPLETION_VALUE"
...
-- ── 2. VW_REPORT_PROJECT_DETAIL_STRUCTURE — fix BT2 leaf display ─────────────
...
SELECT
  ...
  -- BT2: all TEC is earned; BT1: use recorded completion
  CASE WHEN ps."BILLING_TYPE_ID" = 2
       THEN COALESCE(ps."REVENUE", 0)
       ELSE COALESCE(ps."REVENUE_COMPLETION", 0) + COALESCE(ps."EXTRAS_COMPLETION", 0)
  END                                                                       AS "EARNED_VALUE_NET",

  COALESCE(ps."REVENUE", 0) + COALESCE(ps."EXTRAS", 0)                     AS "HONORAR_NET",

  -- BT2: rest = 0 (all earned); BT1: budget minus completion
  CASE WHEN ps."BILLING_TYPE_ID" = 2
       THEN 0
       ELSE (COALESCE(ps."REVENUE", 0) + COALESCE(ps."EXTRAS", 0))
          - (COALESCE(ps."REVENUE_COMPLETION", 0) + COALESCE(ps."EXTRAS_COMPLETION", 0))
  END                                                                       AS "REST_HONORAR",

  -- BT2: always 100% if has budget; BT1: recorded percent
  CASE WHEN ps."BILLING_TYPE_ID" = 2
       THEN CASE WHEN COALESCE(ps."REVENUE", 0) > 0 THEN 100.0 ELSE 0 END
       ELSE COALESCE(ps."REVENUE_COMPLETION_PERCENT", 0)
  END                                                                       AS "LEISTUNGSSTAND_PERCENT",

  CASE
    WHEN (CASE WHEN ps."BILLING_TYPE_ID" = 2
               THEN COALESCE(ps."REVENUE", 0)
               ELSE COALESCE(ps."REVENUE_COMPLETION", 0) + COALESCE(ps."EXTRAS_COMPLETION", 0)
          END) = 0 THEN NULL
...
-- ── 3. fn_project_report_header — full date-filtered fix ─────────────────────
...
LANGUAGE sql STABLE AS $$
  WITH
    -- Snapshot cutoff: end-of-day for Stichtag; end of period for Zeitraum
    cutoff AS (
      SELECT
        CASE
    ...
    -- All leaf structures for this project
    leaf_structs AS (
      SELECT
        ps."ID"              AS "STRUCTURE_ID",
        ps."BILLING_TYPE_ID",
    ...
    -- Budget at cutoff: last PP row with non-null REVENUE ≤ cutoff.ts
    -- Falls back to current PROJECT_STRUCTURE values if no PP row exists yet.
    budget AS (
      SELECT
        ls."STRUCTURE_ID",
        ls."BILLING_TYPE_ID",
        COALESCE(pp_bud."REVENUE", ls."REV_FALLBACK", 0) AS "REVENUE",
        COALESCE(pp_bud."EXTRAS",  ls."EXT_FALLBACK", 0) AS "EXTRAS"
      FROM leaf_structs ls
      LEFT JOIN LATERAL (
        SELECT pp."REVENUE", pp."EXTRAS"
        FROM   public."PROJECT_PROGRESS" pp, cutoff
    ...
    -- Completion at cutoff for BT1: last PP row with non-null REVENUE_COMPLETION ≤ cutoff.ts
    -- Queried separately so a later PAYED/INVOICED row (which has NULL REVENUE_COMPLETION)
    -- does not zero out the completion.
    completion AS (
      SELECT
        ls."STRUCTURE_ID",
    ...
    -- TEC per leaf (date-filtered) — used for BT2 completion and project KPIs
    tec_leaf AS (
      SELECT
        t."STRUCTURE_ID",
        COALESCE(SUM(t."QUANTITY_INT"), 0) AS "HOURS_TOTAL",
    ...
    -- Aggregate budget + completion across all leaves
    prog_agg AS (
      SELECT
        COALESCE(SUM(b."REVENUE"), 0) AS "REVENUE_BUDGET",
        COALESCE(SUM(b."EXTRAS"),  0) AS "EXTRAS_BUDGET",

        -- BT2: completion = TEC.SP_TOT; BT1: completion from PROJECT_PROGRESS
        COALESCE(SUM(
        ...
        CASE
          WHEN SUM(COALESCE(b."REVENUE", 0)) + SUM(COALESCE(b."EXTRAS", 0)) = 0 THEN NULL
          ELSE 100.0
             * ( SUM(CASE WHEN b."BILLING_TYPE_ID" = 2 THEN COALESCE(tl."SP_TOT", 0)
                          ELSE COALESCE(c."REVENUE_COMPLETION", 0) END)
               + SUM(CASE WHEN b."BILLING_TYPE_ID" = 2 THEN 0
                          ELSE COALESCE(c."EXTRAS_COMPLETION", 0) END) )
             / ( SUM(COALESCE(b."REVENUE", 0)) + SUM(COALESCE(b."EXTRAS", 0)) )
        END AS "LEISTUNGSSTAND_PERCENT"
    ...
    -- Overall project TEC (includes non-leaf structures for completeness)
    tec_agg AS (
      SELECT
    ...
    -- Date-filtered billed amounts: PARTIAL_PAYMENT + INVOICE by document date
    -- Storno documents have negative amounts and naturally cancel their originals.
    pp_billed AS (
...
-- ── 4. fn_project_report_structure — fix BT2 and date-filtered completion ────
...
LANGUAGE sql STABLE AS $$
  WITH
    ...
    -- Budget at cutoff per structure (LATERAL — last non-null REVENUE)
    budget AS (
      SELECT
        ps."ID"              AS "STRUCTURE_ID",
        ps."BILLING_TYPE_ID",
        COALESCE(pp_bud."REVENUE", ps."REVENUE", 0) AS "REVENUE",
        COALESCE(pp_bud."EXTRAS",  ps."EXTRAS",  0) AS "EXTRAS"
      FROM public."PROJECT_STRUCTURE" ps
      LEFT JOIN LATERAL (
        SELECT pp."REVENUE", pp."EXTRAS"
        FROM   public."PROJECT_PROGRESS" pp, cutoff
    ...
    -- Completion at cutoff per BT1 structure (last non-null REVENUE_COMPLETION)
    completion AS (
      SELECT
        ps."ID" AS "STRUCTURE_ID",
        COALESCE(pp_compl."REVENUE_COMPLETION",       0) AS "REVENUE_COMPLETION",
    ...
    -- TEC per structure, date-filtered
    tec AS (
      SELECT
        t."STRUCTURE_ID",
        COALESCE(SUM(t."QUANTITY_INT"), 0) AS "HOURS_TOTAL",
  ...
  SELECT
    ...
    -- BT2: earned = TEC.SP_TOT; BT1: earned = PROJECT_PROGRESS completion
    CASE WHEN b."BILLING_TYPE_ID" = 2
         THEN COALESCE(t."SP_TOT", 0)
         ELSE COALESCE(c."REVENUE_COMPLETION", 0) + COALESCE(c."EXTRAS_COMPLETION", 0)
    END AS "EARNED_VALUE_NET",

    -- Budget: from PROJECT_PROGRESS at cutoff (LATERAL already applied)
    COALESCE(b."REVENUE", 0) + COALESCE(b."EXTRAS", 0) AS "HONORAR_NET",
    ...
    -- Leistungsstand %
    CASE WHEN b."BILLING_TYPE_ID" = 2
         THEN CASE WHEN COALESCE(b."REVENUE", 0) > 0 THEN 100.0 ELSE 0 END
...
-- ── 5. fn_project_list_report — fix BT2 and date-filtered completion ─────────
...
LANGUAGE sql STABLE AS $$
  WITH
    ...
    -- Budget at cutoff per leaf (last non-null REVENUE from PROJECT_PROGRESS)
    budget AS (
      SELECT
        ls."PROJECT_ID",
        ls."STRUCTURE_ID",
    ...
    -- Completion at cutoff for BT1 leaves
    completion AS (
      SELECT
        ls."STRUCTURE_ID",
        COALESCE(pp_compl."REVENUE_COMPLETION", 0) AS "REVENUE_COMPLETION",
    ...
    -- Per-project aggregation of budget + completion
    prog_agg AS (
      SELECT
        b."PROJECT_ID",
        COALESCE(SUM(b."REVENUE"), 0) AS "REVENUE_BUDGET",
    ...
    -- Per-project billed amounts (date-filtered)
    billed_agg AS (
      SELECT
        pp."PROJECT_ID",
    ...
    -- Per-project paid amounts (date-filtered)
    pay_agg AS (
      SELECT
        pay."PROJECT_ID",
