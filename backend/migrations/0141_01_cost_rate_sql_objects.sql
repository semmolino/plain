...
DROP VIEW IF EXISTS "REPORTING"."VW_PROJECT_TIME_AGG";
...
CREATE OR REPLACE FUNCTION public.fn_dashboard_monthly(p_tenant_id bigint)
 RETURNS TABLE("MONTH" text, "HOURS_TOTAL" numeric, "COST_TOTAL" numeric)
 LANGUAGE sql
 STABLE
AS $function$
  select
...
CREATE OR REPLACE FUNCTION public.fn_project_report_header(p_tenant_id bigint, p_project_id bigint, p_as_of timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date)
 RETURNS TABLE("TENANT_ID" bigint, "PROJECT_ID" bigint, "NAME_SHORT" text, "NAME_LONG" text, "PROJECT_STATUS_NAME_SHORT" text, "PROJECT_MANAGER_DISPLAY" text, "COMPANY_NAME" text, "BUDGET_TOTAL_NET" numeric, "LEISTUNGSSTAND_PERCENT" numeric, "LEISTUNGSSTAND_VALUE" numeric, "HOURS_TOTAL" numeric, "COST_TOTAL" numeric, "EARNED_VALUE_NET" numeric, "COST_RATIO" numeric, "REMAINING_BUDGET_NET" numeric, "PARTIAL_PAYMENT_NET_TOTAL" numeric, "INVOICE_NET_TOTAL" numeric, "BILLED_NET_TOTAL" numeric, "OPEN_NET_TOTAL" numeric, "PAYED_NET_TOTAL" numeric, "SALES_TOTAL" numeric, "QTY_EXT_TOTAL" numeric)
 LANGUAGE sql
 STABLE
AS $function$
  WITH
...
CREATE OR REPLACE FUNCTION public.fn_project_report_structure(p_tenant_id bigint, p_project_id bigint, p_as_of timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date)
 RETURNS TABLE("TENANT_ID" bigint, "PROJECT_ID" bigint, "STRUCTURE_ID" bigint, "PARENT_STRUCTURE_ID" bigint, "NAME_SHORT" text, "NAME_LONG" text, "IS_LEAF" boolean, "HOURS_TOTAL" numeric, "COST_TOTAL" numeric, "EARNED_VALUE_NET" numeric, "HONORAR_NET" numeric, "REST_HONORAR" numeric, "LEISTUNGSSTAND_PERCENT" numeric, "KOSTENQUOTE" numeric)
 LANGUAGE sql
 STABLE
AS $function$
  WITH
    ...
    coalesce((
      ...
      where pp."TENANT_ID"  = p."TENANT_ID"
        ...
        and (
          (
            (params.date_from is not null or params.date_to is not null)
            and (params.date_from is null or pp."PARTIAL_PAYMENT_DATE"::date >= params.date_from)
            and (params.date_to   is null or pp."PARTIAL_PAYMENT_DATE"::date <= params.date_to)
          )
          or
          (
            params.date_from is null and params.date_to is null
            and pp."PARTIAL_PAYMENT_DATE"::date <= params.as_of_ts::date
          )
        )
    ),0) as "PARTIAL_PAYMENT_NET_TOTAL",
...
select
  ...
  (e."SHORT_NAME" || case when e."FIRST_NAME" is not null then (': ' || e."FIRST_NAME" || ' ' || coalesce(e."LAST_NAME",'')) else '' end)
    as "PROJECT_MANAGER_DISPLAY",
  ...
  (coalesce(ba."PARTIAL_PAYMENT_NET_TOTAL",0) + coalesce(ba."INVOICE_NET_TOTAL",0) - coalesce(ba."PAYED_NET_TOTAL",0)) as "OPEN_NET_TOTAL",
  ((coalesce(pa."REVENUE_COMPLETION_VALUE",0) + coalesce(pa."EXTRAS_COMPLETION_VALUE",0))
    - (coalesce(ba."PARTIAL_PAYMENT_NET_TOTAL",0) + coalesce(ba."INVOICE_NET_TOTAL",0))) as "ABRECHENBAR_NET"
...
CREATE VIEW "REPORTING"."VW_PROJECT_TIME_AGG" AS
 SELECT ps."TENANT_ID",
    ps."PROJECT_ID",
    sum(COALESCE(t."QUANTITY_INT", 0::numeric)) AS "HOURS_TOTAL",
    sum(COALESCE(t."COST_TOTAL", 0::numeric)) AS "COST_TOTAL",
    sum(COALESCE(t."QUANTITY_EXT", 0::numeric)) AS "QTY_EXT_TOTAL",
    sum(COALESCE(t."SP_TOT", 0::numeric)) AS "SALES_TOTAL"
   FROM "TEC" t
     JOIN "PROJECT_STRUCTURE" ps ON ps."TENANT_ID" = t."TENANT_ID" AND ps."ID" = t."STRUCTURE_ID"
  GROUP BY ps."TENANT_ID", ps."PROJECT_ID";
...
CREATE VIEW "REPORTING"."VW_REPORT_PROJECT_DETAIL_STRUCTURE" AS
 SELECT ps."TENANT_ID",
   ...
   FROM "PROJECT_STRUCTURE" ps
     LEFT JOIN "TEC" t ON t."TENANT_ID" = ps."TENANT_ID" AND t."STRUCTURE_ID" = ps."ID"
     LEFT JOIN "REPORTING"."VW_PROJECT_PROGRESS_LATEST" ppl ON ppl."TENANT_ID" = ps."TENANT_ID" AND ppl."STRUCTURE_ID" = ps."ID"
  GROUP BY ps."TENANT_ID", ps."PROJECT_ID", ps."ID", ps."FATHER_ID", ps."NAME_SHORT", ps."NAME_LONG", ppl."REVENUE_COMPLETION", ppl."EXTRAS_COMPLETION";
