...
AS $function$
  select
    to_char(date_trunc('month', t."BOOKING_DATE"::date), 'YYYY-MM') as "MONTH",
    coalesce(sum(t."QUANTITY_INT"), 0)                               as "HOURS_TOTAL",
    coalesce(sum(t."COST_TOTAL"),       0)                               as "COST_TOTAL"
  from public."BOOKING" t
  where t."TENANT_ID" = p_tenant_id
    and coalesce(t."ENTRY_KIND", 'WORK') <> 'BREAK'
    and t."BOOKING_DATE" >= (date_trunc('month', current_date) - interval '5 months')::date
  group by date_trunc('month', t."BOOKING_DATE"::date)
  order by 1
$function$;
...
CREATE OR REPLACE FUNCTION public.protect_arbzg_audit_immutability()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
  ...
  OR COALESCE(NEW."BOOKING_ID", -1) <> COALESCE(OLD."BOOKING_ID", -1) THEN
    RAISE EXCEPTION
...
CREATE VIEW "REPORTING"."VW_PROJECT_TIME_AGG" AS
 SELECT ps."TENANT_ID",
    ...
    sum(COALESCE(t."HOURLY_RATE_TOTAL", 0::numeric)) AS "SALES_TOTAL"
   FROM "BOOKING" t
