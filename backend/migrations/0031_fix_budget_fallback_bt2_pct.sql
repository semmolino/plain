-- Migration 0031: Fix budget fallback and BT2 Leistungsstand % in date-filtered RPCs
--
-- Problems fixed:
--   1. Honorar shows current PROJECT_STRUCTURE budget even when Stichtag is BEFORE
--      the project/structure was created.
--      → Budget LATERAL now checks ps."created_at" ≤ cutoff before falling back to
--        current PROJECT_STRUCTURE.REVENUE (returns 0 if structure didn't exist yet).
--
--   2. fn_project_report_structure shows 100 % Leistungsstand for BT2 leaf nodes
--      regardless of how much TEC was actually booked in the date range.
--      → BT2 LEISTUNGSSTAND_PERCENT now uses actual SP_TOT / REVENUE * 100.
--      → BT2 REST_HONORAR now uses HONORAR_NET - SP_TOT (was hardcoded 0).
--
-- Affects: fn_project_report_header, fn_project_report_structure,
--          fn_project_list_report (all three have the budget fallback issue).
--
...
LANGUAGE sql STABLE AS $$
  WITH
    ...
    -- Budget at cutoff: last PP row with non-null REVENUE ≤ cutoff.
    -- Falls back to current PROJECT_STRUCTURE.REVENUE only if the structure
    -- existed at the cutoff (created_at ≤ cutoff). Returns 0 for future structures.
    budget AS (
      SELECT
        ls."STRUCTURE_ID",
        ls."BILLING_TYPE_ID",
...
LANGUAGE sql STABLE AS $$
  WITH
    ...
    -- Budget at cutoff per structure — only fall back to current PS values if
    -- the structure itself existed at the cutoff (ps."created_at" ≤ cutoff).
    budget AS (
      SELECT
        ps."ID"              AS "STRUCTURE_ID",
        ps."BILLING_TYPE_ID",
  ...
  SELECT
    ...
    -- BT2: rest = budget - SP_TOT; BT1: budget - recorded completion
    (COALESCE(b."REVENUE", 0) + COALESCE(b."EXTRAS", 0))
    - CASE WHEN b."BILLING_TYPE_ID" = 2
           THEN COALESCE(t."SP_TOT", 0)
           ELSE COALESCE(c."REVENUE_COMPLETION", 0) + COALESCE(c."EXTRAS_COMPLETION", 0)
      END AS "REST_HONORAR",

    -- BT2: actual SP_TOT % of budget; BT1: recorded percent
    CASE WHEN b."BILLING_TYPE_ID" = 2
         THEN CASE WHEN COALESCE(b."REVENUE", 0) = 0 THEN 0
                   ELSE 100.0 * COALESCE(t."SP_TOT", 0) / b."REVENUE"
              END
