...
module.exports = (supabase) => {
  ...
  router.get("/me/mastery", async (req, res) => {
    try {
      const svc = require("../services/mastery");
      const r = await svc.computeMastery(supabase, { tenantId: req.tenantId, employeeId: req.employeeId });
      res.json({ data: r });
    } catch (e) {
      res.status(e?.status || 500).json({ error: e?.message || String(e) });
    }
  ...
  router.get("/me/achievements", async (req, res) => {
    try {
      ...
      const r = await svc.evaluateAndList(supabase, { tenantId: req.tenantId, employeeId: req.employeeId, hasFeature: req.hasFeature });
      res.json({ data: r });
    } catch (e) {
      res.status(e?.status || 500).json({ error: e?.message || String(e) });
    }
