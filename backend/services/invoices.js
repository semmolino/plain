...
async function initInvoice(supabase, { companyId, employeeId, projectId, contractId, invoiceType, tenantId }) {
  ...
  if (empErr1) {
    const { data: emp2, error: empErr2 } = await supabase
      .from("EMPLOYEE")
      .select("ID, FIRST_NAME, LAST_NAME, ABBR, MAIL, MOBILE")
      .eq("ID", empId)
      .eq("TENANT_ID", tenantId)
      .maybeSingle();
    ...
    if (rows.length > 0) {
      ...
      if (structureIds.length > 0) {
        ...
        const updates = structureIds.map((sid) => {
          ...
          return { ID: sid, TENANT_ID: tenantId, INVOICED: round2(base + inc) };
        });
